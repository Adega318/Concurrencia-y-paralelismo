...
#define N  500 // Number of bases per sequence
...
int main(int argc, char *argv[] ) {
    ...
    int my_rank, numprocs, acumulado = 0;
    int *recvbuf1, *recvbuf2, *resultp;
    ...
    resultp = malloc(sendcounts[my_rank] / N * sizeof(int));
    ...
    for(i=0; i<sendcounts[my_rank]/N; i++){
        resultp[i] = 0;
        for(j=i * N; j<i*N+N; j++){
    ...
    */
    ...
    MPI_Gatherv(resultp , sendcounts[my_rank], MPI_INT, result, sendcounts, displs, MPI_INT, 0, MPI_COMM_WORLD);
    ...
    free(data1); free(data2); free(result); free(resultp); free(recvbuf1); free(recvbuf2); free(sendcounts); free(displs);
